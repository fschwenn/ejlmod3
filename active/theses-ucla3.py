...
for i in range(numberofpages):
    ...
    tocfilename = '/tmp/THESES-%s-%s.%i.%i.toc' % (abbr.upper(), ejlmod3.stampoftoday(), articlesperpage, i)
...
for rec in recs:
    ...
    ejlmod3.printprogress('-', [[i, len(recs)], [rec['artlink']], [artfilename], [len(relevantrecs)]])
    ...
    for script in artpage.body.find_all('script'):
        ...
        if scriptt:
            ...
            #supervisors
            if 'advisors' in list(scripttjson.keys()):
            ...
            if 'attrs' in list(scripttjson.keys()):
                ...
                #author
                if 'authors' in list(scripttjson['attrs'].keys()):
                    for author in scripttjson['attrs']['authors']:
                        if 'ORCID_id' in list(author.keys()):
    ...
    #sort by subject
    if not 'subjects' in list(rec.keys()):
        rec['subjects'] = ['NoSubject']
    keepit = True
    for s in rec['subjects']:
        subject = re.sub('\W', '', s)
        if subject in subjectstoskip:
            keepit = False
        else:
...
ejlmod3.writenewXML(relevantrecs, publisher, jnlfilename)
