...
if len(sys.argv) > 1 and sys.argv[1] == '-ftp':
    ...
    print('%i files at server, %i files in done, %i files to do' % (len(filesatserver), len(filesdone), len(filestodo)))
    for (i, zf) in enumerate(filestodo):
        print('  %i/%i download %s' % (i+1, len(filestodo), filestodo[i]))
        #srv.get(zf, os.path.join(feeddir, zf))
        f2 = open(os.path.join(feeddir, filestodo[i]), "wb")
        ftp.retrbinary("RETR " + filestodo[i], f2.write)
        f2.close()
    ...
    if os.path.isdir(os.path.join(feeddir, 'tmp', ordner)):
        for ordner2 in os.listdir(os.path.join(feeddir, 'tmp', ordner)):
            for ordner3 in os.listdir(os.path.join(feeddir, 'tmp', ordner, ordner2)):
                ...
                if recs:
                    jnlfilename = 'hindawi%s.%s' % (ordner3, ejlmod3.stampoftoday())
