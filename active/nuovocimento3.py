...
def xmltorec(xmlfilename):
    ...
    xml = BeautifulSoup(''.join(xmlfile.readlines()), features='lxml')
    xmlfile.close()
        ...
        for AuthorGroup in Article.find_all('authorgroup'):
            for a in AuthorGroup.find_all('author'):
                ...
                for fn in a.find_all('familyname'):
                    if re.search('on behalf ', fn.text):
                        coll = re.sub('.*on behalf ', '', fn.text)
                        coll = re.sub('^ *of ', '', coll)
                        coll = re.sub('^ *the ', '', coll)
                        coll = re.sub(' Collaboration', '', coll)
                        artrec['col'] = coll
                        familyname = re.sub(' *on behalf.*', '', fn.text)
                        aut += ' ' + familyname
                    else:
                        aut += ' ' + fn.text
        ...
        for JournalTitle in JournalInfo.find_all('journaltitle'):
            for artrec in artrecs:
                if JournalTitle.text == 'Il Nuovo Cimento C':
                    ...
                    fftprefix = 'ncr'
                #FFT
                if 'licence' in artrec and 'ArticleID' in artrec:
                    artrec['FFT'] = 'http://prometeo.sif.it/papers/?pid=%s%s' % (fftprefix, artrec['ArticleID'])
                    
                    if JournalTitle.text == 'Il Nuovo Cimento C':
                        artrec['FFT'] = 'http://www.sif.it/papers/?pid=%s%s' % (fftprefix, artrec['ArticleID'])
                ejlmod3.printrecsummary(artrec)
    return artrecs
...
os.system('mv %s/%s.zip %s/done/' % (publisherdir, jnlfilename, publisherdir))
