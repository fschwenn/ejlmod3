    ...
    for li in div.find_all('li'):
        ...
        if re.search(r'(?i)pages', lit):
            ...
            if len(p1p2parts) > 1:
                rec['p2'] = p1p2parts[1]
    if not 'p1' in recs:
        for pc in div.find_all('pub-count', attrs = {'count-type' : 'elocation-id'}):
            rec['p1'] = pc.text.strip()
    #vol
    if re.search('ijmpcs',url):
...
for i in range(len(recs)):
        ...
        if 'accordion-tabbed__tab-mobile' in div['class'] or \
            ...
            for div2 in div.find_all('div', attrs = {'class' : 'bottom-info'}):
                ...
                if re.search(r'(?i)COLLABORATION', at):
                    ...
                    recs[i]['col'] = at
                    continue
                elif re.search('orcid.org\/\d\d\d\d', at):
                    autaff.append(re.sub('.*orcid.org\/', 'ORCID:', at))
                ...
                for p in div.find_all('p'):
                    ...
                    if len(pt) > 1:
                        if not re.search('E\-mail Address.*protected', pt):
                            if re.search('orcid.org\/\d\d\d\d', pt):
                                autaff.append(re.sub('.*orcid.org\/', 'ORCID:', pt))
                            else:
                                autaff.append(pt)
    ...
    if not 'abs' in list(recs[i].keys()):
        for div in artpage.body.find_all('div', attrs = {'class' : ['abstractSection', 'abstractInFull', 'abstractinfull']}):
    ...
    #FFT
    if 'license' in list(recs[i].keys()):
