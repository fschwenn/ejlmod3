...
for div in tocpage.body.find_all('div', attrs = {'class' : 'section-container'}):
    ...
    for section in div.find_all('section'):
        ...
        for div2 in section.find_all('div', attrs = {'class' : 'al-article-items'}):
            (artlink, doi) = (False, False)
            for a in div2.find_all('a'):
                if a.has_attr('href'):
                    if re.search('doi.org\/10', a['href']):
                        doi = re.sub('.*doi.org\/', '', a['href'])
                    elif a.has_attr('class') and 'at-articleLink' in a['class']:
                        artlink = "https://academic.oup.com" + a['href']
            if skipalreadyharvested and doi and doi in alreadyharvested:
    ...
    if not articles:
        ...
        #    artlink = "https://academic.oup.com" + a['href']
        #    articles.append((artlink, note))
        for div2 in div.find_all('div', attrs = {'class' : 'al-article-items'}):
            (artlink, doi) = (False, False)
            for a in div2.find_all('a'):
                if a.has_attr('href'):
                    if re.search('doi.org\/10', a['href']):
    ...
    for (artlink, note) in articles:
        i += 1
        ejlmod3.printprogress("-", [[i, len(articles)], [artlink]])
