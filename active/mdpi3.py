...
if jnl == 'proceedings':
    ...
    jnlfilename = 'mdpi_proc%s.%s_%s_%s' % (vol, iss, cnum, ejlmod3.stampoftoday())
    done = []
elif jnl == 'psf':
    starturl = 'http://www.mdpi.com/2673-9984/%s/%s' % (vol, iss)
    jnlfilename = 'mdpi_psf%s.%s_%s_%s' % (vol, iss, cnum, ejlmod3.stampoftoday())
    done = []
else:
    ...
    jnlfilename = '%s.%s_%i' % (jnl, ejlmod3.stampoftoday(), rpp*numberofpages)
    done =  list(map(tfstrip,os.popen("grep '^3.*DOI' %s/backup/*%s*doki |sed 's/.*=//'|sed 's/;//'" % (ejldir, jnl))))
    done +=  list(map(tfstrip,os.popen("grep '^3.*DOI' %s/backup/%4d/*%s*doki |sed 's/.*=//'|sed 's/;//'" % (ejldir, ejlmod3.year()-1, jnl))))
    done +=  list(map(tfstrip,os.popen("grep '^3.*DOI' %s/onhold/*%s*doki |sed 's/.*=//'|sed 's/;//'" % (ejldir, jnl))))


    done +=  list(map(tfstrip,os.popen("grep '^3.*DOI' %s/zu_punkten/*%s*doki |sed 's/.*=//'|sed 's/;//'" % (ejldir, jnl))))
    done +=  list(map(tfstrip,os.popen("grep '^3.*DOI' %s/zu_punkten/enriched/*%s*doki |sed 's/.*=//'|sed 's/;//'" % (ejldir, jnl))))
...
if jnl in ['psf', 'proceedings']:
    ...
    for j in range(numberofpages):
        ejlmod3.printprogress("=", [[j+1, numberofpages], ['%s&page_no=%i' % (starturl, j+1)]])
        req = urllib.request.Request('%s&page_no=%i' % (starturl, j+1), headers=hdr)
        tocpage = BeautifulSoup(urllib.request.urlopen(req), features="lxml")
        divs = tocpage.body.find_all('div', attrs = {'class' : 'article-content'})
        for div in divs:
            doi = False
...
for artlink in artlinks:
    ...
    ejlmod3.printprogress("-", [[i, len(artlinks)], [artlink[0]], [len(recs)]])
    rec['FFT'] = artlink[0] + '/pdf'
    rec['tit'] = artlink[1]
    #get detailed page
    try:
        time.sleep(2)
        artreq = urllib.request.Request(artlink[0], headers=hdr)
        page = BeautifulSoup(urllib.request.urlopen(artreq), features="lxml")
    except:
    ...
    for atype in page.find_all('span', attrs = {'class' : 'label articletype'}):
        ...
        if atype.text == 'Review':
            rec['tc'] = 'R'
    ejlmod3.metatagcheck(rec, page, ['dc.date', 'dc.rights', 'prism.volume', 'prism.number',
                                     'prism.startingPage', 'prism.endingPage', 'citation_doi',
                                     'dc.description'])
    ##keywords
        ...
        for div in page.body.find_all('div', attrs = {'class' : 'belongsTo'}):
            if re.search('Special Issue', div.text):
                for a2 in div.find_all('a'):
                    ...
                    if a2.text in list(conferences.keys()):
                        rec['tc'] = 'C'
    ...
    #        for div in diva.find_all('div', attrs = {'class' : 'author'}):
        ...
        print('              ---{ %s }---' % (reflink))
        try:
            refreq = urllib.request.Request(reflink, headers=hdr)
            refpage = BeautifulSoup(urllib.request.urlopen(refreq))
        except:
    ...
    if (i % chunksize == 0) or (i == len(artlinks)):
        ...
        if recs:
            if i % chunksize == 0:
                xmlfilename = '%s-%02i_of_%i_of_%i' % (jnlfilename, i/chunksize, numberofchunks, maxnumberofchunks)
            else:
                xmlfilename = '%s-fin_of_%i_of_%i' % (jnlfilename, numberofchunks, maxnumberofchunks)
            ejlmod3.writenewXML(recs, publisher, xmlfilename)            
        recs = []
