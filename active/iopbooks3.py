# -*- coding: UTF-8 -*-
# checks for sftp-feeds from IOP for books
# FS 2023-09-12
...
todo = []
srv = pysftp.Connection(host="files.consonance.app", username="iopp_inspire", password="NGklCtUpn0zJx5n7vALtULOE", port=2222)
for datei in srv.listdir():
    if datei in done:
        print('%s already in done' % (datei))
    elif datei[-3:] == 'xml':
        todo.append(os.path.join(iopdirtmp, datei))
        print('downloading %s ...' % (datei))
        srv.get(datei, os.path.join(iopdirtmp, datei))

#scan books
for bookfeed in todo:    
    products = []
    prerecs = {}
    print(bookfeed)
    ...
    for product in products:
        ...
        i += 1
        ejlmod3.printprogress("-", [[i, len(products)]])
                ...
                for pit in pi.find_all('productidtype'):
                    if pit.text == '15':
                        for idv in pi.find_all('idvalue'):
                            ...
                            if 'productform' in list(rec.keys()):
                                rec['isbns'] = [[('a', isbn), ('b', rec['productform'])]]
                            else:
                                rec['isbns'] = [[('a', isbn)]]
                            if rec['doi'] in list(prerecs.keys()):
                ...
                for ttc in ot.find_all(['texttype', 'texttypecode']):
                    if ttc.text in ['02', '03']:
                        for ott in ot.find_all('text'):
                            rec['abs'] = re.sub('<!.CDATA\[', '', re.sub('\]\]>$', '', ott.text))
        #date
        ...
        if not quasiuniquekey in list(prerecs.keys()):
    ...
    recs = list(prerecs.values())
    ejlmod3.writenewXML(recs, publisher, iopf)
    os.system('mv %s %s' % (bookfeed, iopdirdone))

print('done :-)')
    
os.system('rm -rf %s' % (iopdirtmp))
