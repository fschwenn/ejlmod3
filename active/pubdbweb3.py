...
def inspireconf(confsearch):
    ...
    print(searchurl)
    try:
        inspirexml = urllib.request.urlopen(searchurl).read()
...
def requestarticles(timespan):
    ...
    for i in range(numoftimespans):
        ...
        ejlmod3.printprogress("=", [[i+1, numoftimespans], [stampofstartdate, stampofstopdate]])
        ...
        print('  ={ %s }===' % (tocurl))
        pages.append(BeautifulSoup(urllib.request.urlopen(tocurl), features="lxml"))
        try:
            ...
            for pagenum in range(numofpages-1):
                ...
                ejlmod3.printprogress("-", [[pagenum+2, numofpages], [tocurl]])                
                pages.append(BeautifulSoup(urllib.request.urlopen(tocurl), features="lxml"))
        except:
...
def translatearticles(pubdbrecords):
    ...
    for pubdbrecord in pubdbrecords:
        ...
        for cf in pubdbrecord.find_all('controlfield', attrs = {'tag' : '001'}):
            recid = cf.text
            ejlmod3.printprogress("-", [[i, len(pubdbrecords)], [recid]])
        doiindone = False
        confnote = ''
        rec = {'note' : [], 'tc' : '', 'rn' : [], 'MARC' : []}
        #type code
        ...
        for df in pubdbrecord.find_all('datafield', attrs = {'tag' : '856'}):
            ...
            for sf in df.find_all('subfield', attrs = {'code' : 'u'}):
                if re.search('http.*bib\-pubdb1.desy.de.*pdf$', sf.text):
                    ...
                    link = sf.text
            if oa and pdf:
                if ('FFT' not in rec) and (not re.search('title', link)) and rec['tc'] in ['C', 'T', 'K']:
                    #try:
...
if __name__ == '__main__':
    ...
    for tc in list(records.keys()):
        if not tc or not tc[0] in ['T']:
            continue
        #write xml-file
        ejlmod3.writenewXML(records[tc], publisher, jnlfilename)
