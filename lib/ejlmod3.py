...
untitles = ['Calendar', 'Author Index', 'Editorial', 'News', 'Index', 'Spotlights on Recent JACS Publications',
            ...
            'Rear Cover', 'PREFACE', 'Guest Editors', 'Peer review statement']
...
def printprogress(character, information):
    ...
    for subinfo in information:
        ...
        output += 3*character
    print('\033[' + colorcode + 'm' + output + '\033[0m')
    return output
...
def printrecsummary(rec):
    ...
    for k in rec.keys():
        try:
            ...
            output.append('%s' % (k))
    print('\033[' + colorcode + 'm  ', ', '.join(output) + '\033[0m')
    return
def printrec(rec):
    if rec:
        for k in rec:
...
def metatagcheck(rec, artpage, listoftags):
    ...
    #abstract (if there are several in different languages)
