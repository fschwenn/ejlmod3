        ...
        if 'keyw' in rec:
            if len(rec['keyw']) == 1:
                if rekeywsplit1.search(rec['keyw'][0]):
                    ...
                    keywords = re.split('; ', rec['keyw'][0])
                    xmlstring += marcxml('595', [('a', 'Keywords split: "%s"' % (rec['keyw'][0]))])
                elif rekeywsplit3.search(rec['keyw'][0]):
                    keywords = re.split(',', rec['keyw'][0])
                    xmlstring += marcxml('595', [('a', 'Keywords split: "%s"' % (rec['keyw'][0]))])
                else:
                    keywords = rec['keyw']
...
#                xmlstring += marcxml('0247',[('a',pseudodoi), ('2','NODOI'), ('9',publisher)])
        ...
        if 'licence' in rec:
            ...
            elif 'statement' in rec['licence']:
                rec['licence']['statement'] = re.sub(' ', '-', rec['licence']['statement'].upper())
                if re.search('^by.nc', rec['licence']['statement'], re.IGNORECASE):
                    rec['licence']['statement'] = 'CC-' + rec['licence']['statement']
                if re.search('cc.by', rec['licence']['statement'], re.IGNORECASE):
                ...
                for key in thesis['metadata'].keys():
                    ...
                    #PID
                        ...
                        for pid in thesis['metadata'][key]:
                            if re.search('doi.org\/10', pid['value']):
                                ...
                                if fakehdl:
                                    fakedoi = '30.3000/fakehdl/' +  re.sub('.*hdl.handle.net\/', '', pid['value'])
                                else:
                        ...
                        for fac in thesis['metadata'][key]:
                            if fac['value'] in boring:
                                ...
                                rec['fc'] = 'k'
                                                  ...
                                                  'Physikalisches Institut / Lehrstuhl für Experimentalphysik (Astroteilchenphysik)',
                                                  'Erlangen Centre for Astroparticle Physics (ECAP)',
                                                  'Physikalisches Institut / Lehrstuhl für Experimentalphysik (Astroteilchenphysik)']:
                                rec['fc'] = 'a'
                            elif fac['value'] in ['Institute for Theoretical Physics III, Chair for Quantum Gravity']:
                                rec['fc'] = 'g'
                    ...
                    #pages
                        ...
                        for extent in thesis['metadata'][key]:
                            ...
                            elif re.search('\d\d hojas', extent['value']):
                                rec['pages'] = re.sub('.*?(\d\d+) hochas.*', r'\1', extent['value'])
                            else:
...
#                    print("   %i of %i keys not found (%s)" % (len(notdone), len(listofkeys), ', '.join(notdone)))
                if keepit:
                    if 'doi' in rec and rec['doi'] in alreadyharvested:
                        ...
                        if not 'urn' in rec and not 'doi' in rec and fakedoi:
                            rec['doi'] = fakedoi
                        prerecs.append(rec)
